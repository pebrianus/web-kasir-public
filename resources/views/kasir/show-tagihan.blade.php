            ...
            <div class="d-flex justify-content-between align-items-center">
                ...
                <div class="dropdown">
                    <button class="btn btn-outline-primary dropdown-toggle" type="button" data-toggle="dropdown"
                        ...
                        Filter:
                        <strong>{{ $statusFilter == 'proses' ? 'Belum Selesai' : ($statusFilter == 'piutang' ? 'Piutang' : 'Selesai') }}</strong>
                    </button>
                    ...
                    <div class="dropdown-menu dropdown-menu-right">
                        ...
                        <a class="dropdown-item {{ $statusFilter == 'piutang' ? 'active' : '' }}"
                            href="{{ route('kasir.pasien.tagihan', ['norm' => $pasien->NORM, 'jenis_kasir' => $jenis_kasir, 'status' => 'piutang']) }}">
                            Piutang
                        </a>
        ...
        <div class="card-body">
                            ...
                            <div class="col-md-8">
                                ...
                                <ul class="list-unstyled">
                                    ...
                                    <li><strong>Tanggal Tagihan:</strong>
                                        {{ \Carbon\Carbon::parse($tagihan->tgl_tagihan)->format('d F Y H:i') }}
                                    </li>
                                </ul>
                            </div>
                            ...
                            <div class="col-md-4 text-right">
                                ...
                                <div class="text-end">
                                    @if($tagihan->total_diskon > 0)
                                        <small class="text-muted text-decoration-line-through">
                                            Rp {{ number_format($tagihan->total_tagihan_kotor, 2, ',', '.') }}
                                        </small><br>
                                        <small class="text-danger">
                                            Diskon: -Rp {{ number_format($tagihan->total_diskon, 2, ',', '.') }}
                                        </small>
                                    @endif

                                    <h3 class="fw-bold text-success">
                                        Rp {{ number_format($tagihan->total_tagihan_bersih, 2, ',', '.') }}
                                    </h3>
                                </div>

                                {{-- CEK SUDAH ADA TAGIHAN LOKAL ATAU BELUM --}}
                                @if ($processedTags->has($tagihan->no_tagihan))
                                    ...
                                    @if ($tagihanLokal->status_kasir == 'lunas')
                                        <a href="{{ route('kasir.tagihan.lokal', ['id' => $tagihanLokal->id, 'jenis_kasir' => request('jenis_kasir')]) }}"
                                ...
                                @else
                                    ...
                                    <form action="{{ route('kasir.proses-tagihan') }}" method="POST">
                                        ...
                                        <input type="hidden" name="total_asli_simgos" value="{{ $tagihan->total_tagihan_kotor }}">
            ...
            @empty
                <div class="text-center">
                    <p>
                        Tidak ada tagihan dengan status
                        <strong>
                            "{{ $statusFilter == 'proses' ? 'Belum Selesai' : ($statusFilter == 'piutang' ? 'Piutang' : 'Selesai') }}"
                        </strong>.
                    </p>
                </div>
            @endforelse
